...
if ~isempty(parms)
    ...
    type  = upper(parms{5});
    Power_band = gui_GetParms(parms{6}, '%f %f', ';');
    ...
    elseif(strcmpi(fmean, 'N'))
        ...
        for c = 1 : n_chan
            ...
            axis xy;   % x,y축 전환
            %%%%%% 150413 Plot Power %%%%%%%
            Set_Band = double(F >= Power_band(1)) .* double(F < Power_band(2));
            Set_Band_logical = (Set_Band > 0);
            power_single_band = mean(p(Set_Band_logical,:),1);
            subplot(row, col, 2*c+1);
            plot(Head.Stage.Time,power_single_band)
            datetick('x','HH:MM:SS');
            title(Head.ChanLabel{i_chan(c)});
            ylabel('Power(㎶^2/Hz)');
            xlim([stime etime]);
