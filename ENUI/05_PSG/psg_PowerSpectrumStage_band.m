function psg_PowerSpectrumStage_band(handles)
%----------------------------------------------------------
% Pwer Spectrum Stage
%
%
...
parms = inputdlg({'Select Channels',...
                    ...
                    {'C3-M2','W N1 N2 N3 R','0.5 2; 4 7;8 12;13 30;30 45','3','A','max'});
%                     {'F3-M2 C4-M1','W N1 N2 N3 R','0.5 2; 4 7;8 12;13 30;30 45','3','A','1.5'});
...
%     scale = 8 ; % 필요시 이걸로 y축 고정
    ...
    if isempty(spl_length)
        ...
        t1to6 = [1 2 3 4 5 6];
        time_epoch_sequence = t1to6 * n_epoch ; % 이걸 단위별로 subplot할것
        n_plot = 1 ;
    else
        n_epoch = length(Head.Stage.Time);  % epoch max값
        t1to6 = [1 2 3 4 5 6];
...
%                     set(gca,'YScale','log')
% %                     semilogy(F_temp,p,'linewidth',2,'color',Head.Stage.Color{stage});   
...
%             c = 1 ; % 이전 loop문의 잔해, 더미
    ...
    %                     p(b,s) = mean(mean(P(c,i_fband(b,1):i_fband(b,2),epch),2),3);
                ...
                % `150402 excel frame design=========
                excel_frame = [{'Time-range'} {'Frequency'} {'W'} {'N1'} {'N2'} {'N3'} {'R'}];
                xlswrite(['results\' strtok(Head.FileName,'.') '_band.xls'], excel_frame, cell2mat(chan(c)));
                
                Time_range = {[num2str((start_length-1) / 120) , ' ~ ', num2str(end_length / 120), 'h']};
                Time_rangeR = repmat(Time_range,n_plot,1);
                FreqBand = [];
                for n = 1:length(fband(:,1))
                FreqBand = [FreqBand ; {[num2str(fband(n,1)), '~', num2str(fband(n,2)), 'Hz']}];
                end
%                 excel_para1 = [Time_rangeR FreqBand];
                if isempty(spl_length)
                    T_dum(1:5,1) = Time_rangeR;
                    excel_para1 = [T_dum FreqBand];
                else
                    excel_para1 = [Time_rangeR FreqBand];
                end
                cellpos11 = ['A' num2str(t_plot*5-3)];
                xlswrite(['results\' strtok(Head.FileName,'.') '_band.xls'], excel_para1, cell2mat(chan(c)), cellpos11);
                excel_para2 = p;
                cellpos12 = ['C' num2str(t_plot*5-3)];
                xlswrite(['results\' strtok(Head.FileName,'.') '_band.xls'], excel_para2, cell2mat(chan(c)), cellpos12);
